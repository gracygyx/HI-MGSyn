...
class HIMGSyn(torch.nn.Module):
    ...
    def forward(self, drug_feature, drug_adj, ibatch, gexpr_data, adj, druga_id, drugb_id, cellline_id, neg_ids, pos_ids):
        ...
        # drug_lap_loss = laplacian_regularization(drug_embed_set, drug_adj, lambda_coeff=1e-3)
        # print(f'drug_lap_loss: {drug_lap_loss}')
        # hyper_lap_loss = hypergraph_laplacian_regularization(hypergraph_embed, adj, lambda_coeff=1e-3)
        # print(f'hyper_lap_loss: {hyper_lap_loss}')
        # total_loss += drug_lap_loss + hyper_lap_loss
